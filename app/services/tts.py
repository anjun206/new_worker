...
def generate_tts(job_id: str, target_lang: str):
    ...
    for seg in segments:
        ...
        output_file = tts_dir / f"{spk}_{segment_start:.2f}.wav"
        ...
        if COSYVOICE_AVAILABLE:
            ...
            ref_wav_path = tts_dir / f"{spk}_ref.wav"
            # CosyVoice가 파일 입력을 요구할 수 있으므로 참조 샘플을 저장
            speaker_samples[spk].export(str(ref_wav_path), format="wav")
            try:
                ...
                if synthesized_wav:
                    ...
                    ).export(str(output_file), format="wav")
                else:
                    raise RuntimeError(
